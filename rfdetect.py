...
Usage: python3 {argv[0]} -v -k <path to .key files>
	-v                     verbose
...
def main():
	...
	def load_keys(path: str) -> dict:
		...
		key_files = glob(path_join(path, '*.key'))
		for kf_index, kf in enumerate(key_files):
			...
			try:
				...
				ret[basename(kf)] = tuple(bits)
			except Exception as e:
				print(f'Skip key file "{abspath(kf)}" due to parsing error: ' + str(e), file=stderr)
		return ret
	...
	detection_keys: dict[str,tuple[tuple[int,int]]] = load_keys(keys_path)
...
try:
	optlist, args = getopt(argv[1:], 'hHvk:')
