...
python3 "${rfctl_python_path}/rfdump.py" -t 0.5 > "$RFCTL_BIN_FILE_PATH" &&
	python3 "${rfctl_python_path}/rfanalysis.py" -k "$2" "$RFCTL_BIN_FILE_PATH" > "$RFCTL_KEY_FILE_PATH" &&
